...
function renderResume(data, templateId, sectionVisibility) {
  ...
  const vis = {};
  (sectionVisibility || []).forEach(s => { vis[s.key] = s.visible; });

  const css = `--accent: ${data.accentColor || '#d4852a'}; font-size: ${data.fontSize || 12}px`;

  switch (templateId) {
    case 'minimal':    return renderMinimal(data, vis, css);
    case 'executive':  return renderExecutive(data, vis, css);
    case 'creative':   return renderCreative(data, vis, css);
    case 'elegant':    return renderElegant(data, vis, css);
    case 'tech':       return renderTech(data, vis, css);
    default:           return renderModern(data, vis, css);
  }
}
...
function renderBullets(bullets, accentColor) {
  ...
  return `<ul style="list-style:none;padding:0;margin:6px 0 0">
    ${bullets.filter(b => b.trim()).map(b => `
      <li style="display:flex;align-items:flex-start;gap:7px;font-size:11px;color:#333;margin-bottom:3px;line-height:1.55">
...
function renderSkillPills(skills, accentColor, style = 'filled') {
  ...
  if (style === 'outline') {
    return skills.map(s => `<span style="display:inline-block;padding:3px 9px;border-radius:4px;font-size:10.5px;font-weight:500;border:1px solid ${accentColor};color:${accentColor};margin:2px 2px 2px 0;">${escapeHtml(s)}</span>`).join('');
  }
  if (style === 'soft') {
    const hex = accentColor;
    return skills.map(s => `<span style="display:inline-block;padding:3px 9px;border-radius:20px;font-size:10.5px;font-weight:500;background:rgba(0,0,0,0.06);color:#333;margin:2px 2px 2px 0;">${escapeHtml(s)}</span>`).join('');
  }
  return skills.map(s => `<span style="display:inline-block;padding:3px 9px;border-radius:4px;font-size:10.5px;font-weight:500;background:${accentColor};color:white;margin:2px 2px 2px 0;">${escapeHtml(s)}</span>`).join('');
}
    ...
    <div style="margin-bottom:18px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div>
          ...
          <div style="font-size:11.5px;color:#555;margin-top:1px">${escapeHtml(job.company)}${job.location ? ` · ${escapeHtml(job.location)}` : ''}</div>
        </div>
        <div style="font-size:10.5px;color:#888;white-space:nowrap;flex-shrink:0;padding-left:8px">${escapeHtml(job.startDate || '')}${job.endDate ? ` – ${escapeHtml(job.endDate)}` : ''}</div>
      </div>
...
function educationEntries(data, vis) {
  ...
  return (data.education || []).filter(e => e.degree || e.school).map(edu => `
    <div style="margin-bottom:12px">
      <div style="font-weight:600;font-size:12px;color:#111">${escapeHtml(edu.degree)}</div>
      <div style="font-size:11px;color:#555">${escapeHtml(edu.school)}</div>
      <div style="font-size:10.5px;color:#888">${escapeHtml(edu.year)}${edu.gpa ? ` · ${escapeHtml(edu.gpa)}` : ''}</div>
    </div>`).join('');
}

function projectEntries(data, vis, accentColor) {
  if (!vis.projects) return '';
  return (data.projects || []).filter(p => p.name).map(proj => `
    <div style="margin-bottom:14px">
      <div style="font-weight:600;font-size:12px;color:#111">${escapeHtml(proj.name)}${proj.link ? ` <span style="font-size:10px;color:${accentColor};font-weight:400">${escapeHtml(proj.link)}</span>` : ''}</div>
      ${proj.tech ? `<div style="font-size:10.5px;color:${accentColor};margin:2px 0 4px">${escapeHtml(proj.tech)}</div>` : ''}
...
function certEntries(data, vis) {
  ...
  return (data.certifications || []).filter(c => c).map(c => `
    <div style="font-size:11px;color:#444;padding:3px 0;border-bottom:1px solid #f5f5f5">${escapeHtml(c)}</div>`).join('');
}

function langEntries(data, vis, accentColor) {
  if (!vis.languages) return '';
  return (data.languages || []).filter(l => l.name).map(l => `
    <div style="display:flex;justify-content:space-between;align-items:center;font-size:10.5px;margin-bottom:5px;color:#444">
      <span>${escapeHtml(l.name)}</span>
      <span style="font-size:9.5px;color:${accentColor};font-weight:500;letter-spacing:0.04em">${escapeHtml(l.level)}</span>
    </div>`).join('');
}
...
<div class="resume-modern" style="${cssVars}">
  ...
  <div class="resume-header" style="padding:36px 40px 28px;border-bottom:2px solid ${accent};background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);color:white;display:flex;align-items:flex-start;gap:24px">
    ...
    <div style="flex:1">
      <div style="font-family:'Cormorant Garamond',serif;font-size:32px;font-weight:600;line-height:1.1;letter-spacing:-0.5px;color:white">${escapeHtml(data.name) || '<span style="opacity:0.3">Your Name</span>'}</div>
  ...
  <div style="display:grid;grid-template-columns:1fr 200px;min-height:900px">
    ...
    <div style="padding:28px 32px;border-right:1px solid #f0f0f0">
      ${vis.summary && data.summary ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:15px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.1em;border-bottom:1px solid ${accent};padding-bottom:5px;margin-bottom:12px">Summary</div>
        <p style="font-size:11.5px;line-height:1.65;color:#444;margin-bottom:20px">${escapeHtml(data.summary)}</p>` : ''}

      ${vis.experience && (data.experience||[]).filter(j=>j.title||j.company).length ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:15px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.1em;border-bottom:1px solid ${accent};padding-bottom:5px;margin-bottom:14px">Experience</div>
        ${experienceEntries(data, vis, accent)}` : ''}

      ${vis.projects && (data.projects||[]).filter(p=>p.name).length ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:15px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.1em;border-bottom:1px solid ${accent};padding-bottom:5px;margin:20px 0 14px">Projects</div>
        ${projectEntries(data, vis, accent)}` : ''}
    </div>

    <!-- Sidebar -->
    <div style="padding:28px 20px;background:#fafaf9">
      ${vis.education && (data.education||[]).filter(e=>e.degree||e.school).length ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:13px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.08em;border-bottom:1px solid rgba(212,133,42,0.3);padding-bottom:4px;margin-bottom:10px">Education</div>
        ${educationEntries(data, vis)}` : ''}

      ${vis.skills && (data.skills||[]).length ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:13px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.08em;border-bottom:1px solid rgba(212,133,42,0.3);padding-bottom:4px;margin:18px 0 10px">Skills</div>
        <div>${renderSkillPills(data.skills, accent)}</div>` : ''}

      ${vis.certifications && (data.certifications||[]).filter(c=>c).length ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:13px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.08em;border-bottom:1px solid rgba(212,133,42,0.3);padding-bottom:4px;margin:18px 0 10px">Certifications</div>
        ${certEntries(data, vis)}` : ''}

      ${vis.languages && (data.languages||[]).filter(l=>l.name).length ? `
        <div style="font-family:'Cormorant Garamond',serif;font-size:13px;font-weight:600;color:${accent};text-transform:uppercase;letter-spacing:0.08em;border-bottom:1px solid rgba(212,133,42,0.3);padding-bottom:4px;margin:18px 0 10px">Languages</div>
...
function renderMinimal(data, vis, cssVars) {
  const accent = data.accentColor || '#d4852a';
  const ST = (label) => `<div style="font-size:9px;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:${accent};margin:24px 0 10px">${label}</div>`;
  return `
<div class="resume-minimal" style="${cssVars};padding:52px 56px;background:white;font-size:11.5px;color:#1a1a1a">
  <div style="font-family:'Cormorant Garamond',serif;font-size:42px;font-weight:300;letter-spacing:-1px;color:#111;line-height:1">${escapeHtml(data.name) || '<span style="opacity:0.2">Your Name</span>'}</div>
  ...
  ${vis.experience && (data.experience||[]).filter(j=>j.title||j.company).length ? `
    ${ST('Experience')}
    ${(data.experience||[]).filter(j=>j.title||j.company).map(job => `
      <div style="margin-bottom:20px">
        ...
        ${(job.bullets||[]).filter(b=>b.trim()).map(b=>`<div style="font-size:11px;color:#333;line-height:1.6;margin-bottom:3px;padding-left:12px;position:relative"><span style="position:absolute;left:0;color:${accent}">–</span>${escapeHtml(b)}</div>`).join('')}
      </div>`).join('')}` : ''}
...
function renderExecutive(data, vis, cssVars) {
  const accent = data.accentColor || '#d4852a';
  const ST = (label) => `
    <div style="font-family:'Cormorant Garamond',serif;font-size:18px;font-weight:500;color:#111;text-align:center;letter-spacing:0.05em;margin:24px 0 14px;position:relative">
...
<div style="${cssVars};background:white;font-size:11.5px;color:#1a1a1a">
  ...
  <div style="padding:44px 48px 28px;text-align:center;border-bottom:3px solid ${accent}">
    ...
    <div style="font-family:'Cormorant Garamond',serif;font-size:40px;font-weight:500;letter-spacing:-0.5px;color:#111">${escapeHtml(data.name) || '<span style="opacity:0.2">Your Name</span>'}</div>
  ...
  <div style="padding:32px 48px">
    ...
    ${vis.languages && (data.languages||[]).filter(l=>l.name).length ? `${ST('Languages')}<div style="display:flex;flex-wrap:wrap;gap:16px;justify-content:center">${(data.languages||[]).filter(l=>l.name).map(l=>`<span style="font-size:11px;color:#444">${escapeHtml(l.name)} <span style="color:${accent};font-size:9.5px;font-weight:600">${escapeHtml(l.level)}</span></span>`).join('')}</div>` : ''}
  </div>
</div>`;
}
...
function renderCreative(data, vis, cssVars) {
  ...
  const SST = (label) => `<div style="font-size:9px;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:rgba(255,255,255,0.75);margin:16px 0 8px;border-bottom:1px solid rgba(255,255,255,0.2);padding-bottom:4px">${label}</div>`;
  const MST = (label) => `<div style="font-family:'Cormorant Garamond',serif;font-size:16px;font-weight:600;color:${accent};border-bottom:2px solid ${accent};padding-bottom:5px;margin:22px 0 12px">${label}</div>`;
  return `
<div style="${cssVars};background:white;font-size:11.5px;color:#1a1a1a;display:grid;grid-template-columns:200px 1fr;min-height:1123px">
  ...
  <div style="background:${accent};padding:36px 20px;color:white">
    ...
    <div style="font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:600;line-height:1.2;text-align:center;margin-bottom:4px">${escapeHtml(data.name) || 'Your Name'}</div>
    ...
    ${vis.languages && (data.languages||[]).filter(l=>l.name).length ? `
      ${SST('Languages')}
      ${(data.languages||[]).filter(l=>l.name).map(l=>`<div style="display:flex;justify-content:space-between;font-size:10px;margin-bottom:4px;opacity:0.9"><span>${escapeHtml(l.name)}</span><span style="opacity:0.7">${escapeHtml(l.level)}</span></div>`).join('')}` : ''}

    ${vis.certifications && (data.certifications||[]).filter(c=>c).length ? `
      ${SST('Certifications')}
      ${(data.certifications||[]).filter(c=>c).map(c=>`<div style="font-size:10px;opacity:0.9;margin-bottom:4px;line-height:1.5">${escapeHtml(c)}</div>`).join('')}` : ''}
  </div>
...
function renderElegant(data, vis, cssVars) {
  ...
  const ST = (label) => `<div style="font-family:'Cormorant Garamond',serif;font-size:18px;font-weight:400;color:#111;letter-spacing:2px;text-transform:uppercase;text-align:center;margin:22px 0 14px">${label}</div>${DL}`;
  return `
<div style="${cssVars};background:white;font-size:11.5px;color:#2c2c2c;padding:48px 52px">
  ...
  <div style="text-align:center;margin-bottom:24px;padding-bottom:24px;border-bottom:1px solid #e8e0d4">
    ...
    <div style="font-family:'Cormorant Garamond',serif;font-size:46px;font-weight:300;letter-spacing:3px;color:#111;line-height:1">${escapeHtml(data.name) || '<span style="opacity:0.2">Your Name</span>'}</div>
  ...
  ${vis.education && (data.education||[]).filter(e=>e.degree||e.school).length ? `${ST('Education')}${educationEntries(data, vis)}` : ''}
  ${vis.projects && (data.projects||[]).filter(p=>p.name).length ? `${ST('Portfolio')}<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">${(data.projects||[]).filter(p=>p.name).map(proj=>`<div style="padding:12px;border:1px solid #e8e0d4;border-radius:8px"><div style="font-weight:600;font-size:12px;color:#111">${escapeHtml(proj.name)}</div>${proj.tech?`<div style="font-size:10px;color:${accent};margin:3px 0">${escapeHtml(proj.tech)}</div>`:''}${proj.description?`<div style="font-size:11px;color:#555;line-height:1.55">${escapeHtml(proj.description)}</div>`:''}</div>`).join('')}</div>` : ''}
  ${vis.certifications && (data.certifications||[]).filter(c=>c).length ? `${ST('Certifications')}${certEntries(data, vis)}` : ''}
  ${vis.languages && (data.languages||[]).filter(l=>l.name).length ? `${ST('Languages')}<div style="display:flex;flex-wrap:wrap;justify-content:center;gap:14px">${(data.languages||[]).filter(l=>l.name).map(l=>`<span style="font-size:11px;color:#444">${escapeHtml(l.name)} <span style="color:${accent};font-size:9.5px;font-weight:500">${escapeHtml(l.level)}</span></span>`).join('')}</div>` : ''}
</div>`;
}
...
function renderTech(data, vis, cssVars) {
  ...
  const badge = (txt) => `<span style="display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;background:rgba(88,166,255,0.1);border:1px solid rgba(88,166,255,0.2);color:${accent};margin:2px 2px 2px 0;">${escapeHtml(txt)}</span>`;
  const MST = (label) => `<div style="font-size:10px;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:${accent};background:rgba(88,166,255,0.07);padding:4px 10px;border-radius:4px;border-left:3px solid ${accent};margin:20px 0 12px;display:inline-block">${label}</div><br>`;
  const SST = (label) => `<div style="font-size:9px;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:${accent};margin:14px 0 7px;border-bottom:1px solid rgba(88,166,255,0.2);padding-bottom:3px">${label}</div>`;
  return `
<div style="${cssVars};background:white;font-size:11px;color:#1a1a1a">
  ...
  <div style="padding:28px 40px;display:grid;grid-template-columns:1fr 190px;gap:0">
    ...
    <div style="padding-right:28px;border-right:1px solid #f0f0f0">
      ...
      ${vis.experience && (data.experience||[]).filter(j=>j.title||j.company).length ? `${MST('// Experience')}${experienceEntries(data, vis, accent)}` : ''}
      ${vis.projects && (data.projects||[]).filter(p=>p.name).length ? `${MST('// Projects')}${(data.projects||[]).filter(p=>p.name).map(proj=>`
        <div style="margin-bottom:14px">
          ...
          ${proj.tech?`<div style="margin:3px 0">${proj.tech.split(',').map(t=>badge(t.trim())).join('')}</div>`:''}
          ${proj.description?`<div style="font-size:11px;color:#444;line-height:1.55">${escapeHtml(proj.description)}</div>`:''}`).join('')}` : ''}
    </div>
    ...
    <div style="padding-left:24px">
      ...
      ${vis.certifications && (data.certifications||[]).filter(c=>c).length ? `
        ${SST('// Certs')}
        ${(data.certifications||[]).filter(c=>c).map(c=>`<div style="font-size:10.5px;color:#444;padding:3px 0;border-bottom:1px solid #f0f0f0">${escapeHtml(c)}</div>`).join('')}` : ''}
